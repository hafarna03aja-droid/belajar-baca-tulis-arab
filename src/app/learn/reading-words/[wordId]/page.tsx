...
export default function ReadingWordsLessonPage({
  params,
}: {
  ...
  const handlePlay = () => {
    ...
    audio.play().then(() => {
      ...
      if ('speechSynthesis' in window) {
        ...
        // Coba gunakan teks arab untuk pengucapan jika didukung, atau fallback ke transliterasi
        const utteranceText = wordData.word || wordData.transliteration
        const utterance = new SpeechSynthesisUtterance(utteranceText)
        utterance.lang = 'ar-SA' // Suara Arab
        utterance.rate = 0.8 // Perlambat sedikit
        ...
        <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="relative group cursor-pointer" onClick={handlePlay}>
              ...
              <div className="bg-[#0F172A] rounded-[3rem] border border-[#334155] p-12 text-center relative overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex justify-between items-start mb-12 relative z-10">
                  <div className="text-left">
                    <div className="flex gap-2 mb-1">
                      ...
                      <div className="text-[10px] font-extrabold uppercase tracking-widest text-[#3B82F6] bg-[#3B82F6]/10 px-2 rounded">
                        {wordData?.category}
                      </div>
                    </div>
                  ...
                  {/* Play Button */}
                  <button 
                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shadow-lg ${isPlaying ? 'bg-[#10B981] text-white scale-110 shadow-[#10B981]/50' : 'bg-[#1E293B] text-[#10B981] border border-[#334155] hover:border-[#10B981] hover:scale-105'}`}
                  >
                    {isPlaying ? (
                ...
                <div className="relative z-10 py-8">
                  <div className="font-arabic text-center leading-tight mb-8 text-white transition-all duration-500 hover:scale-105 hover:text-[#10B981]" style={{ fontSize: '9rem' }}>
                    {wordData?.word}
                  </div>
                </div>
                ...
                <div className="mt-8 border-t border-[#334155] pt-8 relative z-10">
                  {!revealed ? (
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full text-sm font-bold uppercase tracking-widest text-[#10B981] bg-[#10B981]/10 border border-[#10B981]/30 hover:bg-[#10B981] hover:text-white transition-all group"
                    >
                      Buka Cara Baca & Arti <span className="group-hover:translate-y-1 transition-transform">↓</span>
                    </button>
                  ) : (
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="flex gap-3 mt-auto">
              {prevWord ? (
                <Link href={`/learn/reading-words/${prevWord.id}?level=${level}`} className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-white border border-[#E2E8F0] shadow-sm text-[#0F172A] rounded-2xl hover:border-[#10B981] transition-all text-center">
                  ← {prevWord.transliteration}
                </Link>
              ) : <div className="flex-1" />}
              {nextWord ? (
                 <Link href={`/learn/reading-words/${nextWord.id}?level=${level}`} className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-[#10B981] text-white shadow-[0_4px_14px_rgba(16,185,129,0.39)] hover:scale-[1.02] rounded-2xl transition-all text-center">
