...
import { HIJAIYAH_LETTERS, HARAKATS } from '@/lib/hijaiyah-data'
import { useLearningStore } from '@/store/learningStore'
import Navbar from '@/components/Navbar'

// Audio URLs from a free CDN for Arabic sounds (using text-to-speech placeholder)
const getAudioUrl = (letter: string, harakat: string) => {
  // Use a free TTS API or static audio. For MVP we use the browser's SpeechSynthesis
  return null
}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 pt-24 md:pt-32 pb-16 md:pb-24">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-6 md:mb-8 bg-white p-2 rounded-[1.5rem] md:rounded-[2rem] border border-[#E2E8F0] shadow-[0_4px_20px_rgba(15,23,42,0.03)] w-full sm:w-fit mx-auto lg:mx-0">
          ...
          <button
            ...
            className={`px-4 sm:px-8 py-3 sm:py-3.5 rounded-full text-sm sm:text-[15px] font-bold transition-all w-full sm:w-auto ${quizMode ? 'bg-[#D97706] text-white shadow-[0_4px_15px_rgba(217,119,6,0.4)]' : 'text-[#64748B] hover:bg-[#F1F5F9]'}`}
          >
            🎯 Mode Kompetensi
          </button>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
            ...
            {letter && (level === 2 || level === 3 || level === 4) && (() => {
              ...
              if (level === 2) {
                ...
                // Semua harakat termasuk sukun & tanwin
                pairs.push({ arabic: `${letter.withFathah} ${letter.withKasrah} ${letter.withDhammah}`, label: 'a — i — u' })
