...
// Calculate accuracy using a skeleton-based Precision & Recall algorithm
function calcAccuracy(userCanvas: HTMLCanvasElement, letter: any): number {
  ...
  zoneCtx.lineWidth = 75 // Lebar toleransi drastis diperbesar (lebih mudah di hp)
  ...
  // Syarat lulus dipermudah (sangat friendly untuk touchscreen hp)
  const mappedRecall = Math.min(1, recall / 0.50) // Hanya perlu cover 50% skeleton
  const mappedPrecision = Math.min(1, precision / 0.50) // Toleransi coretan liar lebih tinggi
...
export default function WritingLessonPage({
  ...
  const [strokeWidth, setStrokeWidth] = useState(12) // Default lebih tebal untuk jari (HP)
  ...
  // Draw guide letter on guide canvas
  const drawGuide = useCallback(() => {
    ...
    // Draw the Arabic letter as a large ghost
    ctx.save()
    ...
    // Draw directional arrow (start point indicator)
    if (showGuide) {
      ctx.save()
  ...
  const draw = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Throttle points that are too close to improve rendering performance and curve smoothness
    setCurrentPath((prev) => {
      if (prev.length > 0) {
        const last = prev[prev.length - 1]
        const dist = Math.hypot(last.x - pos.x, last.y - pos.y)
        if (dist < 1.5) return prev // Jarak dikurangi agar lebih responsif di layar sentuh
      }
      return [...prev, pos]
    })
  }
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 animate-fade-in">
            ...
            <div className="bento-card p-8 text-center relative overflow-hidden">
              ...
              <div className="font-arabic text-9xl text-[#3B82F6] mb-4 leading-none py-4">
                {letter?.letter || ''}
              </div>
            ...
            <div className="flex gap-3">
              {prevLetter ? (
                <Link
                  href={`/learn/writing/${prevLetter.id}?level=${level}`}
                  className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-[#1E293B] border border-[#334155] shadow-sm text-white rounded-2xl hover:border-[#3B82F6] transition-all text-center"
                >
                  ← {prevLetter.name}
                </Link>
              ...
              {nextLetter ? (
                <Link
                  href={`/learn/writing/${nextLetter.id}?level=${level}`}
                  className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-[#2563EB] text-white shadow-[0_4px_14px_rgba(37,99,235,0.39)] hover:scale-[1.02] rounded-2xl transition-all text-center"
                >
                  {nextLetter.name} →
                </Link>
              ) : <div className="flex-1" />}
          ...
          <div className="lg:col-span-3 flex flex-col gap-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="bg-[#0F172A] rounded-[2rem] border border-[#334155] overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-b border-[#334155] bg-[#0B1120] gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <div
                      className={`w-9 h-5 rounded-full transition-colors relative ${showGuide ? 'bg-[#3B82F6]' : 'bg-[#334155]'}`}
                ...
                <div className="flex items-center gap-3 bg-[#1E293B] px-3 py-1.5 rounded-full border border-[#334155]">
                  {[8, 12, 16].map((size) => (
                    <button
                      ...
                      className={`rounded-full transition-all ${strokeWidth === size ? 'bg-[#3B82F6] scale-110 shadow-md' : 'bg-[#475569] hover:bg-[#64748B]'}`}
                ...
                {paths.length === 0 && currentPath.length === 0 && (
                  <div className="absolute inset-x-0 bottom-8 flex justify-center pointer-events-none z-0">
                    <p className="text-[11px] font-bold uppercase tracking-widest text-white bg-[#0F172A]/90 backdrop-blur border border-[#334155] px-6 py-2 rounded-full shadow-sm">
                      Mulai goresan dari titik oranye
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-4 p-5 bg-[#0F172A]">
                ...
                <button
                  ...
                  className="w-2/3 py-4 text-sm font-bold uppercase tracking-widest btn-primary disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Validasi Akurasi
                </button>
              </div>
            ...
            <div className="bento-card p-6 bg-gradient-to-br from-[#0F172A] to-[#1E293B]">
              <div className="flex gap-4 items-start">
                <div className="w-10 h-10 rounded-xl bg-[#F59E0B]/20 text-[#FCD34D] flex items-center justify-center text-xl flex-shrink-0">
                  💡
                </div>
                <div>
                  ...
                  <p className="text-[#94A3B8] text-sm leading-relaxed font-medium">
                    Huruf Arab ditulis dari <strong>Kanan ke Kiri</strong>. Jangan terburu-buru, algoritma kami menilai letak <em>overlap</em> tinta Anda terhadap garis bayangan, bukan kecepatan goresan.
                  </p>
                </div>
              </div>
            </div>
