...
function calcAccuracy(userCanvas: HTMLCanvasElement, maskCanvas: HTMLCanvasElement): number {
  ...
  if (precision < 0.05) score -= 40;
  if (score < 0) score = 0;
  return Math.round(score)
}

export default function WritingWordsClient({
  params,
}: {
    ...
    return (
      <div className="min-h-screen bg-[var(--color-bg)] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/learn" className="btn-primary px-6 py-2 mt-4 inline-block text-sm">Kembali ke Peta</Link>
        </div>
      </div>
    )
  }
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 animate-fade-in">
            <div className="bento-card p-8 text-center relative overflow-hidden">
              ...
              <div className="flex justify-center gap-2 mb-4">
                ...
                <div className="text-[10px] font-extrabold tracking-widest text-[#3B82F6] bg-[#3B82F6]/10 px-2 rounded uppercase">{wordData?.category}</div>
              </div>
            ...
            <div className="flex gap-3">
              {prevWord ? (
                <Link href={`/learn/writing-words/${prevWord.id}?level=${level}`} className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-[#1E293B] border border-[#334155] shadow-sm text-white rounded-2xl hover:border-[#10B981] transition-all text-center">← {prevWord.transliteration}</Link>
              ) : <div className="flex-1" />}
              {nextWord ? (
                <Link href={`/learn/writing-words/${nextWord.id}?level=${level}`} className="flex-1 py-4 text-sm font-bold uppercase tracking-wider bg-[#064E3B] text-white shadow-[0_4px_14px_rgba(6,78,59,0.39)] hover:scale-[1.02] rounded-2xl transition-all text-center">{nextWord.transliteration} →</Link>
              ) : <div className="flex-1" />}
            </div>
          </div>
            ...
            <div className="bg-[#0F172A] rounded-[2rem] border border-[#334155] overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
              <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-b border-[#334155] bg-[#0B1120] gap-4">
                ...
                <div className="flex items-center gap-3 bg-[#1E293B] px-3 py-1.5 rounded-full border border-[#334155]">
                  {[4, 8, 12].map((size) => (
                    <button key={size} onClick={() => setStrokeWidth(size)} className={`rounded-full transition-all ${strokeWidth === size ? 'bg-[#10B981] scale-110 shadow-md' : 'bg-[#475569] hover:bg-[#64748B]'}`} style={{ width: size + 6, height: size + 6 }} title={`Ketebalan ${size}`} />
                  ))}
                </div>
              </div>
              <div ref={containerRef} className="relative aspect-video w-full bg-[#1E293B]" style={{ maxHeight: 400 }}>
                ...
                <canvas ref={maskCanvasRef} width={800} height={400} className="hidden" />
                <canvas ref={guideCanvasRef} width={800} height={400} className="absolute inset-0 w-full h-full" />
                <canvas ref={canvasRef} width={800} height={400} className="absolute inset-0 w-full h-full touch-none cursor-crosshair z-10" onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={stopDrawing} />
                {paths.length === 0 && currentPath.length === 0 && (
                  <div className="absolute inset-x-0 bottom-8 flex justify-center pointer-events-none z-0">
                    <p className="text-[11px] font-bold uppercase tracking-widest text-white bg-[#0F172A]/90 backdrop-blur border border-[#334155] px-6 py-2 rounded-full shadow-sm">Tulis dari Kanan ke Kiri</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-4 p-5 bg-[#0F172A]">
                <button onClick={handleClear} className="w-1/3 py-4 text-[13px] font-bold uppercase tracking-widest bg-[#1E293B] border border-[#334155] text-[#94A3B8] rounded-2xl hover:bg-[#334155] hover:border-[#475569] hover:text-white transition-all">Reset</button>
                <button onClick={handleCheck} disabled={paths.length === 0 && currentPath.length === 0} className="w-2/3 py-4 text-sm font-bold uppercase tracking-widest bg-[#064E3B] text-white rounded-2xl hover:bg-[#10B981] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Validasi Akurasi</button>
              </div>
            </div>
          </div>
        </div>
