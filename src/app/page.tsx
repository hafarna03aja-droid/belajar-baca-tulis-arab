  ...
  return (
    <div className="min-h-screen font-sans selection:bg-[#2563EB]/30 bg-[var(--color-bg)]">
      ...
      <section className="relative min-h-[95vh] flex items-center justify-center overflow-hidden mesh-bg pt-20 rounded-b-[3rem] shadow-2xl">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-5 sm:px-6 text-center animate-fade-in mt-10">
          <div className="inline-flex items-center gap-2.5 bg-white/5 border border-white/10 rounded-full px-4 py-2 sm:px-5 sm:py-2 mb-8 sm:mb-10 backdrop-blur-md shadow-xl">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-[5.5rem] font-extrabold text-white leading-[1.1] sm:leading-[1.1] mb-6 sm:mb-8 tracking-tight">
            ...
            <span className="relative inline-block">
              ...
              <svg className="absolute -bottom-2 sm:-bottom-3 left-0 w-full opacity-60" viewBox="0 0 300 12" fill="none">
      ...
      {/* Stats - Floating */}
      <section className="-mt-16 relative z-20 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto glass-premium rounded-3xl p-6 sm:p-8 grid grid-cols-2 sm:flex sm:justify-around gap-6 sm:gap-6 items-center border border-white/10 glow-blue">
          {[
            ...
            <div key={stat.label} className={`text-center group ${stat.extraClasses || ''}`}>
              ...
              <div className="text-[10px] sm:text-[11px] uppercase tracking-widest text-white/50 font-bold mt-1.5 sm:mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            {FEATURES.map((feature, i) => (
              <div
                key={i}
                className={`bento-card p-6 sm:p-8 md:p-10 flex flex-col justify-between group shadow-3d ${feature.span}`}
              >
                <div>
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#F59E0B]/10 flex items-center justify-center text-2xl sm:text-3xl mb-6 sm:mb-8 group-hover:scale-110 transition-transform duration-300">
                    {feature.icon}
                  </div>
                  <h3 className="text-xl sm:text-2xl font-bold text-white mb-2.5 sm:mb-3">{feature.title}</h3>
                  <p className="text-[#94A3B8] leading-relaxed text-base sm:text-lg">{feature.desc}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="flex flex-col gap-5 sm:gap-6 text-left">
                {CURRICULUM.map((item, i) => (
                  <div key={i} className="flex gap-4 sm:gap-5 items-start">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-[#F59E0B]/10 text-[#F59E0B] flex items-center justify-center font-bold text-base sm:text-lg flex-shrink-0 mt-1 sm:mt-0">
                      0{item.level}
                    </div>
                    <div>
                      <h4 className="text-lg sm:text-xl font-bold text-white mb-1">{item.title}</h4>
                      <p className="text-sm sm:text-base text-[#94A3B8]">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* CTA Section */}
      <section className="py-20 px-5 sm:px-6 relative overflow-hidden">
        <div className="max-w-4xl mx-auto text-center">
          <div className="mesh-bg rounded-3xl p-8 sm:p-12 md:p-20 relative overflow-hidden bento-card border-none shadow-2xl">
            <div className="absolute top-4 right-4 sm:right-8 text-6xl sm:text-8xl opacity-10 font-arabic text-white">بسم</div>
            <div className="absolute bottom-4 left-4 sm:left-8 text-6xl sm:text-8xl opacity-10 font-arabic text-white">الله</div>
      ...
      {/* Footer minimal */}
      <footer className="py-10 sm:py-12 border-t border-[#334155] mt-10">
        <div className="max-w-6xl mx-auto px-5 sm:px-6 flex flex-col md:flex-row justify-between items-center gap-5 sm:gap-4 text-center md:text-left">
